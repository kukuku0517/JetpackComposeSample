...
class Time : Serializable {
    ...
    companion object {
        ...
        fun getAge(birthDate: String): Int {
            val birth = Time(birthDate)
            val now = Time()

            val yearPassed =
                if (birth.clone().set(Calendar.YEAR, now.get(Calendar.YEAR)) < now) 0 else -1

            val birthYear = birth.get(Calendar.YEAR)
            val nowYear = now.get(Calendar.YEAR)

            return nowYear - birthYear + yearPassed
        }

        fun fromDate(date: Date): Time {
            return Time(date)
        }

        fun formatMillis(format: String = "HH:mm:ss", millis: Long): String {
            val h = millis / 1000 / 60 / 60 % 24
            val m = millis / 1000 / 60 % 60
            val s = millis / 1000 % 60
            return format
                .replace("HH",String.format("%02d",h))
                .replace("mm",String.format("%02d",m))
                .replace("ss",String.format("%02d",s))
        }
    }
    ...
    fun minusMillis(time: Time): Long {
        return toTimeMillis() - time.toTimeMillis()
    }
